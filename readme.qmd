...
**NOTE:** Zeitlicher Abstand aktuell = 0 –\> größerer Abstand eröffnet Debatte um Artefakt vs. Handlung
...
**TODO:** in Beispieldaten Murmuras in TRACKING APP umbennen
...
plot1 = ggplot(most_visited_apps, aes(x = reorder(app_name, visit), y = visit)) +
  ...
  labs(
    ...
    subtitle = "Data based on a Sample of German Internet Users (N = XXX)",
    x = "App Name",
    y = "Visits"
  )
...
plot3 = ggplot(instagram_participants, aes(x = metric, y = value, fill = metric)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  scale_fill_manual(
    values = c("visits" = "#5E81AC", "duration" = "#BF616A"),
...
```{r}
# TODO: Nutzungsverhalten über Zeit pro Nutzer in einem Plot für mehrere Nutzer. Soll die temporale variance zwischen Tagen und Nutzer zeigen. Mal bei Multilevel Analysen anschauen. Als Tipp für Data Exploration bevor die Analyse stattfindet

random_sample_15 = blue_data_clean %>% 
  select(panelist_id) %>% 
  distinct() %>% 
  slice_sample(n = 25) %>% 
  pull(panelist_id)
...
instagram_participants_days = blue_data_clean %>% 
  # filter for Instagram usage
  filter(app_name == "Instagram") %>% 
  filter(panelist_id %in% random_sample_15) %>% 
...
# Berechne den Mittelwert pro Teilnehmer
participant_means = instagram_participants_days %>%
  group_by(panelist_id) %>%
  summarise(mean_value = mean(value, na.rm = TRUE)) 

# Violin-Plot mit sortierten Teilnehmern
ggplot(instagram_participants_days, 
       aes(x = fct_reorder(panelist_id, value, .fun = mean, .desc = TRUE),
           y = value,
           fill = metric)) +
  geom_boxplot(width = 0.5, outlier.size = 1, alpha = 0.8) +
  facet_wrap(~ metric, scales = "free_y") +
  scale_fill_manual(values = c("duration" = "#BF616A")) +
...
plot3 = ggplot(week, aes(x = date, y = value, color = metric)) +
...
plot4 = ggplot(hourly_data, aes(x = hour, y = value)) +
    geom_area(fill = "#BF616A", alpha = 0.4) +   # Fläche unter der Linie
  geom_line(color = "#BF616A", linewidth = 1) +
  geom_point(color = "#BF616A", size = 2) +
...
colors =  c("#5E81AC", "#B48EAD", "#BF616A", "#D08770","#EBCB8B") 
...
Let's take a closer look at the event_types. Here, we can refer to the article by Parry & Toth (2025), the official [Android Developers Documentation](https://developer.android.com/reference/android/app/usage/UsageEvents.Event#constants_1) or [Android Code Search](https://cs.android.com/?hl=de)(search for "UsageEvents"), which breaks down the meaning of each type.
...
```{r}
plot6 = ggplot(most_visited_apps, aes(x = reorder(app_name, visit), y = visit)) +
  geom_col(width = 0.6, fill = "#5E81AC") +
  coord_flip() +
  geom_text(aes(label = visit), hjust = -0.2, size = 3, family = "Times New Roman") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
...
plot7 = ggplot(shares, aes(x = reorder(app_name, n_shares), y = n_shares)) +
  geom_col(width = 0.6, fill = "#5E81AC") +
  coord_flip() +
...
plot8 = ggplot(notifications, aes(x = reorder(app_name, n_notifications), y = n_notifications)) +
  geom_col(width = 0.6, fill = "#5E81AC") +
  coord_flip() +
