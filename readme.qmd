...
blacklisted_apps = readRDS("data/blacklisted_apps.rds") %>% select(-app_package)
```
...
```{r}
instagram_week = blue_data_clean %>% 
  filter(app_name == "Instagram") %>% 
  group_by(date) %>% 
  summarise(
    visits = n(),
...
plot3 = ggplot(instagram_week, aes(x = date, y = value, color = metric)) +
  ...
  labs(
    title = "Instagram usage over a week",
    subtitle = "Data based on a Sample of German Internet Users (N = XXX)",
    x = "Date",
    y = "Value"
  )
...
instagram_day = blue_data_clean %>% 
  # we filter our data based on the previously selected random day and Instagram
  filter(app_name == "Instagram" & date == random_day) %>% 
  # we need the hour of the day for our analysis
  mutate(
    # let's make sure that we have the right timezone
    hour = as.POSIXct(start_time, tz = "Europe/Berlin"),
    # and round the timestamps to hours
    hour = floor_date(hour, unit = "hour")
  ) %>% 
  group_by(hour) %>% 
  summarise(
    visits = n(),
    # Note: The accumulated duration may exceed one hour, as we perform the calculation for several people and do not include the end time.
    duration = sum(duration, na.rm = TRUE) / 60 # in minutes
  ) %>% 
  pivot_longer(cols = c(visits, duration),
               names_to = "metric",
...
plot4 = ggplot(instagram_day, aes(x = hour, y = value, color = metric)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_color_manual(
    values = c("visits" = "#5E81AC", "duration" = "#BF616A"),
  ...
  labs(
    title = "Instagram usage over the course of a day",
    subtitle = "Data based on a Sample of German Internet Users (N = XXX)",
    x = "Time",
    y = "Value"
  )
...
gaps = instagram_visual %>%
  ...
  select(panelist_id, app_name, gap_start, gap_end, session_id) %>%
  rename(
...
plot5 = ggplot(instagram_visual) +
  ...
  labs(
    x = "Time",
    y = NULL,
    title = "A random Instagram session on a Smartphone",
  ) +
  theme_minimal(base_size = 13, base_family = "Times New Roman") +
  theme(
    legend.title = element_blank(),
